...
best = [2141.3747, 650.78, 77.0894892,214.13747, 7, 1200, 2141.3747]

#Get Feature inout from users

def get_user_input():
    wm = st.number_input('Wm (in µm)', min_value = Wm[0], max_value = Wm[-1], value = best[0], step = (Wm[1] - Wm[0]))
    w0m  = st.number_input('W0m (in µm)', min_value = W0m[0], max_value = W0m[-1], value = best[1], step = (W0m[1] - W0m[0]))
    d    = st.number_input('dm (in µm)', min_value = dm[0], max_value = dm[-1], value = best[2], step = (dm[1] - dm[0]))
    t    = st.number_input('tm (in µm)', min_value = tm[0], max_value = tm[-1], value = best[3], step = (tm[1] - tm[0]))
    rows = st.number_input('Rows', min_value = Rows[0], max_value = Rows[-1], value = best[4], step = (Rows[1] - Rows[0]))
    xa   = st.number_input('Xa (in µm)', min_value = Xa[0], max_value = Xa[-1], value = best[5], step = (Xa[1] - Xa[0]))
    ya   = st.number_input('Ya (in µm)', min_value = Ya[0], max_value = Ya[-1], value = best[6], step = (Ya[1] - Ya[0]))
...
if __name__ == '__main__':
    sys.argv = ["streamlit", "run", "WebApp1.py"]
    sys.exit(stcli.main())
